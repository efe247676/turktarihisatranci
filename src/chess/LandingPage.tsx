    ...
    <div className="min-h-screen w-full overflow-x-hidden bg-stone-950 text-amber-50">
      <nav
        className={`fixed left-0 right-0 top-0 z-50 transition-all duration-300 ${
          scrolled
            ? 'border-b border-amber-800/40 bg-stone-950/90 shadow-lg backdrop-blur-md'
            : 'bg-transparent'
        }`}
      >
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3 sm:px-6">
          ...
          <button
            onClick={onPlay}
            className="rounded-lg bg-gradient-to-br from-amber-500 to-amber-700 px-4 py-2 text-sm font-bold text-amber-50 shadow-lg transition hover:from-amber-400 hover:to-amber-600 active:scale-95 sm:px-5"
          >
            Oyna
          </button>
        </div>
      ...
      <section
        ...
        style={{
          backgroundImage:
            'radial-gradient(circle at 20% 20%, rgba(180, 83, 9, 0.24), transparent 50%), radial-gradient(circle at 80% 80%, rgba(6, 78, 59, 0.28), transparent 50%), linear-gradient(135deg, #1c1917, #292524, #0c0a09)',
        }}
      >
        <div
        ...
        <div className={`relative z-10 max-w-3xl text-center ${mounted ? 'hero-in' : 'opacity-0'}`}>
          ...
          <p className="mx-auto mt-5 max-w-xl text-base text-amber-100/80 sm:text-lg">
            Osmanlı, Timurlu ve Safevi dönemlerinden ilham alan ücretsiz bir tarayıcı satranç oyunu.
            Tarihi savaş atmosferinde yapay zekaya karşı veya arkadaşınla çevrimiçi oyna.
          </p>
          <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <button
              onClick={onPlay}
              className="group relative w-full overflow-hidden rounded-xl border-2 border-amber-400 bg-gradient-to-br from-amber-500 to-amber-700 px-8 py-4 text-lg font-black tracking-wide text-amber-50 shadow-2xl shadow-amber-500/40 transition hover:shadow-amber-400/60 active:scale-95 sm:w-auto"
            >
            ...
            <button
              onClick={onOnline}
              className="w-full rounded-xl border-2 border-blue-400/60 bg-blue-950/40 px-8 py-4 text-lg font-bold tracking-wide text-blue-100 backdrop-blur transition hover:border-blue-400 hover:bg-blue-900/40 active:scale-95 sm:w-auto"
            >
              Çevrimiçi Oyna
            </button>
          </div>
          <p className="mt-6 text-xs text-amber-200/50">
            Ücretsizdir, kayıt gerektirmez ve modern tarayıcılarda çalışır.
          </p>
        </div>
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          <div className="mb-12 text-center">
            ...
            <p className="mx-auto mt-3 max-w-2xl text-amber-100/60">
              Klasik satrancı Türk tarihiyle buluşturan özgün bir strateji deneyimi.
            </p>
          </div>
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {[
              { title: 'Tarihi Savaşlar', desc: 'Ankara 1402 ve Çaldıran 1514 muharebelerini satranç tahtasında tematik olarak canlandırır.' },
              { title: 'Yapay Zeka Rakibi', desc: 'Kolay, orta ve zor seçenekleriyle tek başına oynanabilen yerel maçlar sunar.' },
              { title: 'Çevrimiçi Oyun', desc: 'Davet bağlantısıyla arkadaşınla aynı tahtada buluşabilir, hızlıca maç başlatabilirsin.' },
              { title: 'Tam Kurallar', desc: 'Rok, geçerken alma, terfi, şah, mat ve pat durumları desteklenir.' },
              { title: 'Mobil Uyumlu', desc: 'Telefon, tablet ve bilgisayarda kurulum gerektirmeden çalışacak şekilde tasarlanmıştır.' },
              { title: 'Eğitici İçerik', desc: 'Oyun, tarih merakını stratejiyle birleştiren güvenli ve aile dostu bir deneyim hedefler.' },
            ].map((f) => (
              <div
                key={f.title}
                className="rounded-lg border border-amber-900/40 bg-stone-800/70 p-6 shadow-lg transition hover:border-amber-600/60"
              >
                <h3 className="mb-2 text-lg font-bold text-amber-200">{f.title}</h3>
                <p className="text-sm leading-6 text-amber-100/65">{f.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="battles" className="bg-stone-950 py-16 sm:py-24">
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          <div className="mb-12 text-center">
            ...
            <p className="mx-auto mt-3 max-w-2xl text-amber-100/60">
              İki büyük muharebe, iki farklı çağ. Her kampanya kendi atmosferiyle oynanır.
            </p>
          </div>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            {Object.values(CAMPAIGNS).map((c) => (
              <article
                key={c.id}
                className={`relative overflow-hidden rounded-lg border-2 bg-gradient-to-br ${c.bgGradient} p-6 shadow-2xl sm:p-8 ${
      ...
      <section id="how" className="border-t border-amber-900/30 bg-stone-900 py-16 sm:py-24">
        <div className="mx-auto max-w-4xl px-4 sm:px-6">
          <div className="mb-12 text-center">
            ...
            <p className="mx-auto mt-3 max-w-2xl text-amber-100/60">
              Üç kısa adımda oyuna başlayabilirsin.
            </p>
          </div>
          <div className="space-y-6">
            {[
              { n: '1', title: 'Savaşını Seç', desc: 'Ankara 1402 veya Çaldıran 1514 kampanyasından birini seç.' },
              { n: '2', title: 'Taşını Oynat', desc: 'Bir taşa dokun; yasal hamleler gösterilir. Hedef kareye dokunarak hamleni yap.' },
              { n: '3', title: 'Şah Mat Et', desc: 'Rakip şahı savunmasız bırak, mat kur ve maçı kazan.' },
            ].map((s) => (
              <div key={s.n} className="flex gap-4 rounded-lg border border-amber-900/40 bg-stone-800/60 p-5 sm:gap-6 sm:p-6">
                ...
                <div>
                  ...
                  <p className="text-sm leading-6 text-amber-100/65 sm:text-base">{s.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="bg-stone-900 py-6" aria-label="Reklam alanı">
        <div className="mx-auto max-w-4xl px-4 sm:px-6">
      ...
      <section id="about" className="bg-stone-950 py-16 sm:py-24">
        <div className="mx-auto max-w-4xl px-4 text-center sm:px-6">
          ...
          <p className="mx-auto mt-5 max-w-2xl text-amber-100/70 sm:text-lg">
            ...
            kurallarını Türk tarihinden esinlenen temalarla birleştiren bağımsız bir web oyunudur.
            Amaç, eğlenceli bir strateji deneyimi sunarken tarih merakını canlı tutmaktır.
          </p>
          <p className="mx-auto mt-4 max-w-2xl text-amber-100/60">
            Oyun ücretsizdir. Maç başlatmak için üyelik gerekmez. Çevrimiçi davet özelliği yalnızca
            oyuncuları aynı odaya bağlamak için kullanılır.
          </p>
        </div>
      </section>

      <section
        className="relative overflow-hidden border-y border-amber-800/40 py-16 sm:py-20"
      ...
      >
        <div className="mx-auto max-w-3xl px-4 text-center sm:px-6">
          ...
          <p className="mx-auto mt-4 max-w-xl text-amber-100/70 sm:text-lg">
            Tahtaya otur, ordunu kur ve stratejinle zafere ulaş.
          </p>
          <button
            onClick={onPlay}
            className="group relative mt-8 overflow-hidden rounded-xl border-2 border-amber-400 bg-gradient-to-br from-amber-500 to-amber-700 px-12 py-4 text-xl font-black tracking-wide text-amber-50 shadow-2xl shadow-amber-500/40 transition hover:shadow-amber-400/60 active:scale-95"
          >
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          <div className="flex flex-col items-center justify-between gap-6 md:flex-row">
            ...
            <div className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-sm text-amber-100/60">
              ...
              <a href="/gizlilik.html" className="transition hover:text-amber-300">Gizlilik</a>
              <a href="/kullanim-sartlari.html" className="transition hover:text-amber-300">Kullanım</a>
              <a href="/iletisim.html" className="transition hover:text-amber-300">İletişim</a>
            </div>
          </div>
          <div className="mt-8 border-t border-amber-900/30 pt-6 text-center text-xs leading-6 text-amber-200/45">
            <p>© 2026 Türk Tarihi Satrancı. Tüm hakları saklıdır.</p>
            <p>Eğitim ve eğlence amacıyla hazırlanmış bağımsız bir satranç oyunudur.</p>
          </div>
        </div>
      </footer>
    </div>
